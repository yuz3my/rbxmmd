...
<div class="wrap">
  ...
  <section id="sec-settings" style="display:none">
    ...
    <div class="field">
      <label for="smoothInput">Suavização: <span id="smoothVal" class="mono">0</span>%</label>
      <input type="range" id="smoothInput" min="0" max="90" step="1" value="0">
      <div class="hint">Se o boneco tremer/vibrar no lugar, sobe esse valor. Ele filtra ruído da rotação entre um frame e outro — muito alto deixa o movimento mole/atrasado, então vai testando aos poucos.</div>
    </div>
    <div class="switch-row">
      <div>
...
(function(){
  ...
  exportBtn.addEventListener("click", ()=>{
    ...
    const smoothPct = parseFloat(smoothInput.value) || 0;
    const smoothKeep = 1 - (smoothPct/100)*0.95; // how much of the NEW sample survives each step
    const smoothState = {};
    function smooth(key, raw){
      if(smoothPct<=0) return raw;
      if(!smoothState[key]){ smoothState[key] = raw; return raw; }
      const s = qSlerp(smoothState[key], raw, smoothKeep);
      smoothState[key] = s;
      return s;
    }
    ...
    if(durationSec - times[times.length-1] > stepSec*0.25) times.push(durationSec);
    ...
    for(const t of times){
      ...
      const lowerTorsoQ = smooth('lowerTorso', limbQuat("lowerBody"));
      let upperTorsoQ = limbQuat("upperBody");
      if(matched.upperBody2){
        upperTorsoQ = qMul(upperTorsoQ, limbQuat("upperBody2"));
      }
      upperTorsoQ = smooth('upperTorso', upperTorsoQ);
      let headQ = limbQuat("neck");
      headQ = qMul(headQ, limbQuat("head"));
      headQ = smooth('head', headQ);

      const zero = {x:0,y:0,z:0};
      const leftHand = poseXML("LeftHand", smooth('leftWrist', limbQuat("leftWrist")), zero, []);
      const leftLowerArm = poseXML("LeftLowerArm", smooth('leftElbow', limbQuat("leftElbow")), zero, [leftHand]);
      const leftUpperArmQ = smooth('leftArm', retargetJoint(limbQuat("leftArm"), leftShoulderDiff));
      const leftUpperArm = poseXML("LeftUpperArm", leftUpperArmQ, zero, [leftLowerArm]);
      const rightHand = poseXML("RightHand", smooth('rightWrist', limbQuat("rightWrist")), zero, []);
      const rightLowerArm = poseXML("RightLowerArm", smooth('rightElbow', limbQuat("rightElbow")), zero, [rightHand]);
      const rightUpperArmQ = smooth('rightArm', retargetJoint(limbQuat("rightArm"), rightShoulderDiff));
      ...
      const leftFoot = poseXML("LeftFoot", smooth('leftAnkle', limbQuat("leftAnkle")), zero, []);
      const leftLowerLeg = poseXML("LeftLowerLeg", smooth('leftKnee', limbQuat("leftKnee")), zero, [leftFoot]);
      const leftUpperLeg = poseXML("LeftUpperLeg", smooth('leftLeg', limbQuat("leftLeg")), zero, [leftLowerLeg]);
      const rightFoot = poseXML("RightFoot", smooth('rightAnkle', limbQuat("rightAnkle")), zero, []);
      const rightLowerLeg = poseXML("RightLowerLeg", smooth('rightKnee', limbQuat("rightKnee")), zero, [rightFoot]);
      const rightUpperLeg = poseXML("RightUpperLeg", smooth('rightLeg', limbQuat("rightLeg")), zero, [rightLowerLeg]);
